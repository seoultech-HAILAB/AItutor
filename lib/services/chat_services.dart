...
class ChatServices {
  ...
  Future<List> endStressConversation(String chatModelKey, String uid, String userName, String apiKey, String chatType) async {
    try {
      ...
      // evaluate prompt
      List evalPromptRes = await getEvalulatePrompt(key: chatModelKey);
      if (!evalPromptRes.first) return evalPromptRes;
      String evaluatePrompt = evalPromptRes.last.replaceAll('<context>', context);
      String scores = await evaluateChat(context, apiKey, evaluatePrompt);

      // summary prompt
      List sumPromptRes = await getSummaryPrompt(key: chatModelKey);
      if (!sumPromptRes.first) return sumPromptRes;
      String summaryPrompt = sumPromptRes.last.replaceAll('<context>', context);
      String summary = await evaluateChat(context, apiKey, summaryPrompt);

      // feedback prompt
      List feedbackPromptRes = await getFeedbackPrompt(key: chatModelKey);
      if (!feedbackPromptRes.first) return feedbackPromptRes;
      String feedbackPrompt = feedbackPromptRes.last.replaceAll('{summary}', summary);
      String feedback = await evaluateChat(summary, apiKey, feedbackPrompt);
